...
namespace filament
{
    namespace viewer
    {
        ...
        {
            if (0 == compare(tokens[i], jsonChunk, "NONE"))
            {
                *out = AntiAliasing::NONE;
            }
            else if (0 == compare(tokens[i], jsonChunk, "FXAA"))
            {
        ...
        {
            if (0 == compare(tokens[i], jsonChunk, "ADD"))
            {
                *out = BlendMode::ADD;
            }
            else if (0 == compare(tokens[i], jsonChunk, "INTERPOLATE"))
            {
                *out = BlendMode::INTERPOLATE;
            }
            else
            {
        ...
        {
            if (0 == compare(tokens[i], jsonChunk, "LOW"))
            {
                *out = VQL::LOW;
            }
            else if (0 == compare(tokens[i], jsonChunk, "MEDIUM"))
            {
                *out = VQL::MEDIUM;
            }
            else if (0 == compare(tokens[i], jsonChunk, "HIGH"))
            {
                *out = VQL::HIGH;
            }
            else if (0 == compare(tokens[i], jsonChunk, "ULTRA"))
            {
                *out = VQL::ULTRA;
            }
            else
        ...
        {
            if (0 == compare(tokens[i], jsonChunk, "LINEAR"))
            {
                *out = ToneMapping::LINEAR;
            }
            else if (0 == compare(tokens[i], jsonChunk, "ACES_LEGACY"))
            {
                *out = ToneMapping::ACES_LEGACY;
            }
            else if (0 == compare(tokens[i], jsonChunk, "ACES"))
            {
                *out = ToneMapping::ACES;
            }
            else if (0 == compare(tokens[i], jsonChunk, "FILMIC"))
            {
                *out = ToneMapping::FILMIC;
            }
            else if (0 == compare(tokens[i], jsonChunk, "UCHIMURA"))
            {
                *out = ToneMapping::UCHIMURA;
            }
            else if (0 == compare(tokens[i], jsonChunk, "REINHARD"))
            {
                *out = ToneMapping::REINHARD;
            }
            else if (0 == compare(tokens[i], jsonChunk, "DISPLAY_RANGE"))
            {
                *out = ToneMapping::DISPLAY_RANGE;
            }
            else
            {
        ...
        static int parse(jsmntok_t const *tokens, int i, const char *jsonChunk, Dithering *out)
        {
            ...
            else if (0 == compare(tokens[i], jsonChunk, "TEMPORAL"))
            {
                *out = Dithering::TEMPORAL;
            }
            else
            {
        ...
        {
            if (0 == compare(tokens[i], jsonChunk, "PCF"))
            {
                *out = ShadowType::PCF;
            }
            else if (0 == compare(tokens[i], jsonChunk, "VSM"))
            {
                *out = ShadowType::VSM;
            }
            else
        ...
        {
            for (auto prop : settings.scalar)
            {
                apply(prop, dest);
            }
            for (auto prop : settings.float3)
            {
                apply(prop, dest);
            }
            for (auto prop : settings.float4)
            {
                apply(prop, dest);
            }
        }
        ...
        std::string writeJson(const MaterialSettings &in)
        {
            ...
            oss << "{\n";
            oss << "\"scalar\": {\n";
            for (auto prop : in.scalar)
            {
                writeJson(prop, oss);
            }
            oss << "},\n";
            oss << "\"float3\": {\n";
            for (auto prop : in.float3)
            {
                writeJson(prop, oss);
            }
            oss << "},\n";
            oss << "\"float4\": {\n";
            for (auto prop : in.float4)
            {
                writeJson(prop, oss);
            }
            oss << "},\n";
