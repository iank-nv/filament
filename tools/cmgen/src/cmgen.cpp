...
static int handleCommandLineArgments(int argc, char *argv[])
{
    ...
    while ((opt = getopt_long(argc, argv, OPTSTR, OPTIONS, &option_index)) >= 0)
    {
        ...
        case 'r':
            ...
            {
                std::cerr << "roughness (blur) parameter must be between 0.0 and 1.0" << std::endl;
                exit(0);
            }
            break;
        case 'y':
    ...
    if (g_sh_file != ShFile::SH_NONE || g_debug)
    {
        ...
        if (g_debug)
        {
            ...
            { // save a file with the "other one" (irradiance or radiance)
                std::unique_ptr<filament::math::float3[]> sh = CubemapSH::computeSH(js, inputCubemap, g_sh_compute, !g_sh_irradiance);
                CubemapSH::renderSH(js, cm, sh, g_sh_compute);
                std::string basename = iname.getNameWithoutExtension();
                utils::Path filePath =
                    outputDir + (basename + "_sh" + (!g_sh_irradiance ? "_i" : "_r") + ".hdr");
                        ...
                        const utils::Path &dir)
{
    ...
    for (size_t level = 0; level < numLevels; level++)
    {
        ...
        for (size_t i = 0; i < 6; i++)
        {
            ...
            std::string filename = outputDir + ("is_m" + std::to_string(level) + "_" + CubemapUtils::getFaceName(face) + ext);
    ...
    bool prefilter, const utils::Path &dir)
{
    ...
    for (ssize_t i = baseExp; i >= ssize_t((baseExp + 1) - numLevels); --i)
    {
        ...
        for (size_t j = 0; j < 6; j++)
        {
            ...
            std::string filename = outputDir + ("m" + std::to_string(level) + "_" + CubemapUtils::getFaceName(face) + ext);
            saveImage(filename, g_format, dst.getImageForFace(face), g_compression);
        }
    }
...
static void exportKtxFaces(KtxBundle &container, uint32_t miplevel, const Cubemap &cm)
{
    ...
    for (uint32_t j = 0; j < 6; j++)
    {
        ...
        {
        case 0:
            face = Cubemap::Face::PX;
            break;
        case 1:
            face = Cubemap::Face::NX;
            break;
        case 2:
            face = Cubemap::Face::PY;
            break;
        case 3:
            face = Cubemap::Face::NY;
            break;
        case 4:
            face = Cubemap::Face::PZ;
            break;
        case 5:
            face = Cubemap::Face::NZ;
            break;
        default:
            face = Cubemap::Face::PX;
            break; // make linters happy
        }
