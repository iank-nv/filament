...
#define MATERIAL(n) MATERIALS_##n##_DATA, MATERIALS_##n##_SIZE
                                                                  ...
                                                                  const RenderPass &pass, uint32_t width, uint32_t height, float scale) noexcept
    {
        ...
        auto &structurePass = fg.addPass<StructurePassData>(
            "Structure Pass",
            [&](FrameGraph::Builder &builder, auto &data) {
                data.depth = builder.createTexture("Structure Buffer", {.width = width, .height = height, .levels = uint8_t(levelCount), .format = TextureFormat::DEPTH32F});

                data.depth = builder.write(builder.read(data.depth));

                data.rt = builder.createRenderTarget("Structure Target", {.attachments = {{}, data.depth},
    ...
    */
                                                                   ...
                                                                   FrameGraphId<FrameGraphTexture> input, size_t level) noexcept
    {
        ...
        auto &depthMipmapPass = fg.addPass<DepthMipData>(
            "Depth Mipmap Pass",
            [&](FrameGraph::Builder &builder, auto &data) {
                const char *name = builder.getName(input);
                data.in = builder.sample(input);
                data.out = builder.write(data.in);
                data.rt = builder.createRenderTarget(name, {.attachments = {{}, {data.out, uint8_t(level + 1)}}});
            },
     ...
     */
        ...
        auto &SSAOPass = fg.addPass<SSAOPassData>(
            ...
            [&](FrameGraph::Builder &builder, auto &data) {
                ...
                data.ssao = builder.createTexture("SSAO Buffer", {.width = desc.width,
                                                                  .height = desc.height,
                ...
                data.rt = builder.createRenderTarget("SSAO Target", {.attachments = {data.ssao, data.depth},
                                                                     .clearColor = {1.0f},
            ...
            [=](FrameGraphPassResources const &resources,
                ...
                FMaterialInstance *const mi = material.getMaterialInstance();
                mi->setParameter("depth", depth, {.filterMin = SamplerMinFilter::NEAREST_MIPMAP_NEAREST});
                ...
                const float3 l = normalize(
                    mat3f::getTransformForNormals(m.upperLeft()) * options.ssct.lightDirection);
        ...
        TextureFormat format, BilateralPassConfig config) noexcept
    {
        ...
        auto &blurPass = fg.addPass<BlurPassData>(
            "Separable Blur Pass",
            [&](FrameGraph::Builder &builder, auto &data) {
                ...
                data.blurred = builder.createTexture("Blurred output", {.width = desc.width, .height = desc.height, .format = format});
                ...
                data.rt = builder.createRenderTarget("Blurred target", {.attachments = {data.blurred, depth},
                                                                        .clearColor = {1.0f},
                                                                        .clearFlags = TargetBufferFlags::COLOR});
            },
            [=](FrameGraphPassResources const &resources,
                                                                         ...
                                                                         bool reinhard, size_t kernelWidth, float sigmaRatio) noexcept
    {
        ...
        auto &gaussianBlurPasses = fg.addPass<BlurPassData>(
            ...
            [&](FrameGraph::Builder &builder, auto &data) {
                ...
                data.tempRT = builder.createRenderTarget("Horizontal temporary target", {.attachments = {data.temp}});

                data.outRT = builder.createRenderTarget("Blurred target", {.attachments = {{data.out, dstLevel}}});
            },
            [=](FrameGraphPassResources const &resources,
                auto const &data, DriverApi &driver) {
                ...
                mi->setParameter("source", hwIn, {.filterMag = SamplerMagFilter::LINEAR, .filterMin = SamplerMinFilter::LINEAR_MIPMAP_NEAREST});
                ...
                mi->setParameter("source", hwTemp, {
                                                       .filterMag = SamplerMagFilter::LINEAR, .filterMin = SamplerMinFilter::LINEAR /* level is always 0 */
                                                   });
     ...
     */
        ...
        auto &ppDoFDownsample = fg.addPass<PostProcessDofDownsample>(
            "DoF Downsample",
            [&](FrameGraph::Builder &builder, auto &data) {
                data.color = builder.sample(input);
                data.depth = builder.sample(depth);

                data.outForeground = builder.createTexture("dof foreground output", {.width = width,
                                                                                     .height = height,
                                                                                     .levels = mipmapCount,
                                                                                     .format = format});
                data.outBackground = builder.createTexture("dof background output", {.width = width,
                                                                                     .height = height,
                                                                                     .levels = mipmapCount,
                                                                                     .format = format});
                data.outCocFgBg = builder.createTexture("dof CoC output", {.width = width,
                                                                           .height = height,
                                                                           .levels = mipmapCount,
                ...
                data.rt = builder.createRenderTarget("DoF Target", {.attachments = {
     ...
     */
        ...
        assert(mipmapCount - 1 <= sizeof(PostProcessDofMipmap::rt) / sizeof(FrameGraphRenderTargetHandle));

        auto &ppDoFMipmap = fg.addPass<PostProcessDofMipmap>(
            "DoF Mipmap",
            [&](FrameGraph::Builder &builder, auto &data) {
                ...
                for (size_t i = 0; i < mipmapCount - 1u; i++)
                {
                    ...
                    data.rt[i] = builder.createRenderTarget("DoF Target", {.attachments = {
     ...
     */
        ...
        {
            auto &ppDoFTiling = fg.addPass<PostProcessDofTiling1>(
                "DoF Tiling",
                [&](FrameGraph::Builder &builder, auto &data) {
                    assert((tileBufferWidth & 1u) == 0);
                    assert((tileBufferHeight & 1u) == 0);
                    data.inCocMaxMin = builder.sample(inTilesCocMaxMin);
                    data.outTilesCocMaxMin = builder.createTexture("dof tiles output", {.width = tileBufferWidth >> i,
                                                                                        .height = tileBufferHeight >> i,
                                                                                        .format = TextureFormat::RG16F});
                    data.outTilesCocMaxMin = builder.write(data.outTilesCocMaxMin);
                    data.rt = builder.createRenderTarget("DoF Tiles Target", {.attachments = {data.outTilesCocMaxMin}});
                },
                [=](FrameGraphPassResources const &resources,
                    auto const &data, DriverApi &driver) {
     ...
     */
        ...
        auto dilate = [&](FrameGraphId<FrameGraphTexture> input) -> FrameGraphId<FrameGraphTexture> {
            ...
            auto &ppDoFDilate = fg.addPass<PostProcessDofDilate>(
                "DoF Dilate",
                [&](FrameGraph::Builder &builder, auto &data) {
                    ...
                    data.outTilesCocMaxMin = builder.write(data.outTilesCocMaxMin);
                    data.rt = builder.createRenderTarget("DoF Dilated Tiles Target", {.attachments = {data.outTilesCocMaxMin}});
                },
                [=](FrameGraphPassResources const &resources,
                    auto const &data, DriverApi &driver) {
                    auto const &out = resources.get(data.rt);
     ...
     */
        ...
        auto &ppDoF = fg.addPass<PostProcessDof>(
            "DoF",
            [&](FrameGraph::Builder &builder, auto &data) {
                ...
                data.outForeground = builder.createTexture("dof color output", {.width = (colorDesc.width + 1) / 2,
                                                                                .height = (colorDesc.height + 1) / 2,
                                                                                .format = fg.getDescriptor(data.foreground).format});
                data.outAlpha = builder.createTexture("dof alpha output", {.width = (colorDesc.width + 1) / 2,
                                                                           .height = (colorDesc.height + 1) / 2,
                                                                           .format = TextureFormat::R8});
                data.outForeground = builder.write(data.outForeground);
                data.outAlpha = builder.write(data.outAlpha);
                data.rt = builder.createRenderTarget("DoF Target", {.attachments = {{data.outForeground,
                                                                                     data.outAlpha,
                                                                                     {},
                                                                                     {}},
                                                                                    {},
     ...
     */
        ...
        auto &ppDoFMedian = fg.addPass<PostProcessDofMedian>(
            "DoF Median",
            [&](FrameGraph::Builder &builder, auto &data) {
     ...
     */
        ...
        auto &ppDoFCombine = fg.addPass<PostProcessDofCombine>(
            "DoF combine",
            [&](FrameGraph::Builder &builder, auto &data) {
                data.color = builder.sample(input);
                ...
                data.rt = builder.createRenderTarget("DoF Target", {.attachments = {data.output}});
            },
            [=](FrameGraphPassResources const &resources,
                auto const &data, DriverApi &driver) {
                                                                  ...
                                                                  View::BloomOptions &bloomOptions, float2 scale) noexcept
    {
        ...
        if (2 * width < desc.width || 2 * height < desc.height)
        {
            ...
            input = opaqueBlit(fg, input, {.width = desc.width / 2, .height = desc.height / 2, .format = outFormat});
        }
        ...
        // downsample phase
        auto &bloomDownsamplePass = fg.addPass<BloomPassData>(
            "Bloom Downsample",
            [&](FrameGraph::Builder &builder, auto &data) {
                data.in = builder.sample(input);
                data.out = builder.createTexture("Bloom Texture", {.width = width,
                                                                   .height = height,
                ...
                {
                    data.outRT[i] = builder.createRenderTarget("Bloom target", {.attachments = {{data.out, uint8_t(i)}}});
                }
            },
            [=](FrameGraphPassResources const &resources,
                auto const &data, DriverApi &driver) {
                ...
                for (size_t i = 0; i < bloomOptions.levels; i++)
                {
                    ...
                    mi->setParameter("source", hwOut, {.filterMag = SamplerMagFilter::LINEAR, .filterMin = SamplerMinFilter::LINEAR_MIPMAP_NEAREST});
        ...
        // upsample phase
        auto &bloomUpsamplePass = fg.addPass<BloomPassData>(
            "Bloom Upsample",
            [&](FrameGraph::Builder &builder, auto &data) {
                data.in = builder.sample(input);
    ...
    static float4 getVignetteParameters(View::VignetteOptions options, uint32_t width, uint32_t height)
    {
        if (options.enabled)
        {
            ...
            float midPoint = (1.0f - options.midPoint) * mix(2.2f, 3.0f, oval) * (1.0f - 0.1f * options.feather);
                                                        ...
                                                        bool dithering, uint32_t width, uint32_t height) noexcept
    {
        ...
        FMaterialInstance *mi = material.getMaterialInstance();
        mi->setParameter("lut", colorGrading->getHwHandle(), {.filterMag = SamplerMagFilter::LINEAR, .filterMin = SamplerMinFilter::LINEAR});
    ...
    void PostProcessManager::colorGradingSubpass(DriverApi &driver, bool translucent) noexcept
    {
        ...
        const uint8_t variant = uint8_t(translucent ? PostProcessVariant::TRANSLUCENT : PostProcessVariant::OPAQUE);
                                                                     ...
                                                                     View::BloomOptions bloomOptions, View::VignetteOptions vignetteOptions, bool dithering) noexcept
    {
        ...
        if (bloomOptions.enabled)
        {
            ...
            if (bloomOptions.dirt)
            {
                ...
                bloomDirt = fg.import("dirt", {.width = (uint32_t)fdirt->getWidth(0u), .height = (uint32_t)fdirt->getHeight(0u), .format = fdirt->getFormat()}, frameGraphTexture);
            }
        }

        auto &ppColorGrading = fg.addPass<PostProcessColorGrading>(
            "colorGrading",
            [&](FrameGraph::Builder &builder, auto &data) {
                auto const &inputDesc = fg.getDescriptor(input);
                data.input = builder.sample(input);
                data.output = builder.createTexture("colorGrading output", {.width = inputDesc.width,
                                                                            .height = inputDesc.height,
                                                                            .format = outFormat});
                data.output = builder.write(data.output);
                data.rt = builder.createRenderTarget("colorGrading Target", {.attachments = {data.output}});
            ...
            [=](FrameGraphPassResources const &resources, auto const &data, DriverApi &driver) {
                ...
                mi->setParameter("bloomBuffer", bloomTexture, {
                                                                  .filterMag = SamplerMagFilter::LINEAR, .filterMin = SamplerMinFilter::LINEAR /* always read base level in shader */
                                                              });
                mi->setParameter("dirtBuffer", dirtTexture, {.filterMag = SamplerMagFilter::LINEAR, .filterMin = SamplerMinFilter::LINEAR});
                                                             ...
                                                             TextureFormat outFormat, bool translucent) noexcept
    {
        ...
        auto &ppFXAA = fg.addPass<PostProcessFXAA>(
            "fxaa",
            [&](FrameGraph::Builder &builder, auto &data) {
                auto const &inputDesc = fg.getDescriptor(input);
                data.input = builder.sample(input);
                data.output = builder.createTexture("fxaa output", {.width = inputDesc.width,
                                                                    .height = inputDesc.height,
                                                                    .format = outFormat});
                data.output = builder.write(data.output);
                data.rt = builder.createRenderTarget("FXAA Target", {.attachments = {data.output}});
            },
            [=](FrameGraphPassResources const &resources,
                auto const &data, DriverApi &driver) {
                ...
                FMaterialInstance *mi = material.getMaterialInstance();
                mi->setParameter("colorBuffer", texture, {.filterMag = SamplerMagFilter::LINEAR, .filterMin = SamplerMinFilter::LINEAR});
                                                            ...
                                                            ColorGradingConfig colorGradingConfig) noexcept
    {
        ...
        auto &taa = fg.addPass<TAAData>(
            ...
            [&](FrameGraph::Builder &builder, auto &data) {
                ...
                {
                    data.tonemappedOutput = builder.createTexture("Tonemapped Buffer", {.width = desc.width,
                                                                                        .height = desc.height,
                                                                                        .format = colorGradingConfig.ldrFormat});
                    data.tonemappedOutput = builder.write(data.tonemappedOutput);
                    data.output = builder.read(data.output);
                }
                data.rt = builder.createRenderTarget("TAA target", {.attachments = {{data.output, data.tonemappedOutput, {}, {}}, {}, {}}});
            },
...
#pragma nounroll
                ...
                mi->setParameter("history", history, {.filterMag = SamplerMagFilter::LINEAR, .filterMin = SamplerMinFilter::LINEAR});
                ...
                const uint8_t variant = uint8_t(colorGradingConfig.translucent ? PostProcessVariant::TRANSLUCENT : PostProcessVariant::OPAQUE);
                                                                   ...
                                                                   SamplerMagFilter filter) noexcept
    {
        ...
        auto &ppBlit = fg.addPass<PostProcessScaling>(
            "blit scaling",
            [&](FrameGraph::Builder &builder, auto &data) {
                auto const &inputDesc = fg.getDescriptor(input);
                ...
                data.srt = builder.createRenderTarget(builder.getName(data.input), {.attachments = {data.input},
                ...
                data.output = builder.write(data.output);
                data.drt = builder.createRenderTarget("Scaled Target", {.attachments = {data.output}});
            },
            [=](FrameGraphPassResources const &resources, auto const &data, DriverApi &driver) {
        ...
        FrameGraphTexture::Descriptor outDesc) noexcept
    {
        ...
        auto &ppQuadBlit = fg.addPass<QuadBlitData>(
            ...
            [=](FrameGraphPassResources const &resources,
                ...
                FMaterialInstance *const mi = material.getMaterialInstance();
                mi->setParameter("color", color, {.filterMag = SamplerMagFilter::LINEAR, .filterMin = SamplerMinFilter::LINEAR});
                mi->setParameter("resolution",
                                 float4{desc.width, desc.height, 1.0f / desc.width, 1.0f / desc.height});
                mi->commit(driver);
                mi->use(driver);
                                                                ...
                                                                const char *outputBufferName, FrameGraphId<FrameGraphTexture> input) noexcept
    {
        ...
        auto &ppResolve = fg.addPass<ResolveData>(
            "resolve",
            [&](FrameGraph::Builder &builder, auto &data) {
                ...
                data.srt = builder.createRenderTarget(builder.getName(input), {.attachments = {colorAttachmentSrc, depthAttachmentSrc}, .samples = desc.samples});
                ...
                data.drt = builder.createRenderTarget(outputBufferName, {.attachments = {colorAttachmentDst, depthAttachmentDst}});
            },
            [](FrameGraphPassResources const &resources, auto const &data, DriverApi &driver) {
                auto in = resources.get(data.srt);
                auto out = resources.get(data.drt);
                                                                      ...
                                                                      FrameGraphId<FrameGraphTexture> input, uint8_t layer, size_t level) noexcept
    {
        ...
        auto &depthMipmapPass = fg.addPass<VsmMipData>(
            ...
            [&](FrameGraph::Builder &builder, auto &data) {
                ...
                data.rt = builder.createRenderTarget(name, {.attachments = {{data.out, uint8_t(level + 1), layer}},
            ...
            [=](FrameGraphPassResources const &resources,
                ...
                FMaterialInstance *const mi = material.getMaterialInstance();
                mi->setParameter("color", in, {.filterMag = SamplerMagFilter::LINEAR, .filterMin = SamplerMinFilter::LINEAR_MIPMAP_NEAREST});
                mi->setParameter("level", uint32_t(level));
