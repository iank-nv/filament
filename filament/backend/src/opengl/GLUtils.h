...
#define CHECK_GL_ERROR(out)                                        \
    {                                                              \
        GLUtils::checkGLError(out, __PRETTY_FUNCTION__, __LINE__); \
    }
#define CHECK_GL_FRAMEBUFFER_STATUS(out, target)                                     \
    {                                                                                \
        GLUtils::checkFramebufferStatus(out, target, __PRETTY_FUNCTION__, __LINE__); \
    }
#endif
        ...
        {
            return mode == backend::SamplerCompareMode::NONE ? GL_NONE : GL_COMPARE_REF_TO_TEXTURE;
        }
        ...
        constexpr inline GLenum getFormat(backend::PixelDataFormat format) noexcept
        {
            ...
            case PixelDataFormat::UNUSED:
                return GL_RGBA; // should never happen (used to be rgbm)
